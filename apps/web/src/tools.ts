...
export const tools: readonly ToolDefinition[] = [
  ...
  { id: "raster.crop", kind: "raster", icon: "⌗", iconFile: "МОНТАЖ.svg", label: { en: "Crop Tool", ru: "Рамка" }, shortcut: "C", options: [{ id: "ratio", label: { en: "Ratio", ru: "Соотношение" }, type: "select", defaultValue: "unconstrained", values: [{ value: "unconstrained", label: { en: "Unconstrained", ru: "Свободно" } }, { value: "original", label: { en: "Original ratio", ru: "Исходное соотношение" } }, { value: "1:1", label: { en: "1:1" } }, { value: "4:5", label: { en: "4:5" } }, { value: "5:7", label: { en: "5:7" } }, { value: "2:3", label: { en: "2:3" } }, { value: "16:9", label: { en: "16:9" } }] }, { id: "deleteCroppedPixels", label: { en: "Delete Cropped Pixels", ru: "Удалять обрезанные пиксели" }, type: "boolean", defaultValue: false },
    // Dragging a handle past the canvas edge with this on extends the canvas instead of being
    // clamped to it, and the new border is filled by whichever model is picked here — the
    // owner's own two controls together (docs/master-plan.md §52.8), not a separate "Allow
    // Canvas Extension" toggle: extension with nothing to fill the new area would just be a
    // transparent border, which "Delete Cropped Pixels" already covers for the opposite case.
    { id: "aiBorderFill", label: { en: "AI Border Fill", ru: "ИИ заливка границ" }, type: "boolean", defaultValue: false },
    { id: "aiFillModel", label: { en: "Model", ru: "Модель" }, type: "select", defaultValue: "mi-gan-512", values: [
      { value: "mi-gan-512", label: { en: "MI-GAN 512 (fast)", ru: "MI-GAN 512 (быстрая)" } },
      { value: "lama", label: { en: "LaMa (better)", ru: "LaMa (лучше)" } },
    ] }] },
  { id: "raster.clone", kind: "raster", icon: "⧉", iconFile: "ШТАМП.svg", label: { en: "Clone Tool", ru: "Штамп" }, shortcut: "S", options: [...brushTipOptions, opacity, { id: "pressureSize", label: { en: "Pen pressure: size", ru: "Нажим: размер" }, type: "boolean", defaultValue: true }, { id: "pressureOpacity", label: { en: "Pen pressure: opacity", ru: "Нажим: непрозрачность" }, type: "boolean", defaultValue: false }, { id: "alignMode", label: { en: "Alignment", ru: "Выравнивание" }, type: "select", defaultValue: "registered", values: [{ value: "none", label: { en: "None", ru: "Нет" } }, { value: "registered", label: { en: "Registered", ru: "Совместное" } }, { value: "fixed", label: { en: "Fixed", ru: "Фиксированное" } }] }] },
  ...
  { id: "raster.patch", kind: "raster", icon: "⊞", iconFile: "заплатка.svg", label: { en: "Patch Tool", ru: "Заплатка" }, shortcut: "J", options: [{ id: "mode", label: { en: "Mode", ru: "Режим" }, type: "select", defaultValue: "source", values: [{ value: "source", label: { en: "Source", ru: "Источник" } }, { value: "destination", label: { en: "Destination", ru: "Назначение" } }] }, { id: "feather", label: { en: "Feather", ru: "Растушёвка" }, type: "number", min: 0, max: 250, step: 1, defaultValue: 0, unit: "px" }, { id: "sampleAllLayers", label: { en: "Sample all layers", ru: "Все слои" }, type: "boolean", defaultValue: false }] },
