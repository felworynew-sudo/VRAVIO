import { vectorCorePanels } from "./registry";

const STORAGE_KEY = "vravio.vector-panels.visible";
export const PANEL_REQUEST_EVENT = "vravio-vector-panel-visibility-request";
export const PANEL_CHANGED_EVENT = "vravio-vector-panel-visibility-changed";

function defaults(): Set<string> { return new Set(vectorCorePanels.filter((panel) => panel.defaultVisible).map((panel) => panel.id)); }
