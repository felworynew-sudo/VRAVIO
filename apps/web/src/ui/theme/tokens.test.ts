...
const ALLOWED: readonly { readonly match: string; readonly reason: string }[] = [
  ...
  { match: "scene3d-rotation-knob", reason: "canvas overlay over artwork — same convention as transform-/puppet- handles" },
...
describe("theme tokens", () => {
  ...
  it("defines every token that a rule reads", () => {
    ...
    for (const runtime of ["--tool-mask", "--panel-mask", "--icon-mask", "--mask-link-mask", "--link-badge-mask", "--swap-colors-mask", "--clip-mask", "--accent", "--swatch", "--layer-depth", "--clip-width", "--ant-period", "--vector-zoom"]) defined.add(runtime);
