...
const ALLOWED: readonly { readonly match: string; readonly reason: string }[] = [
  ...
  { match: "select-object-", reason: "the Object Selection tool's box/point handles are drawn over artwork, same convention as transform- handles and puppet-" },
...
describe("theme tokens", () => {
  ...
  it("defines every token that a rule reads", () => {
    ...
    for (const runtime of ["--tool-mask", "--panel-mask", "--icon-mask", "--mask-link-mask", "--link-badge-mask", "--swap-colors-mask", "--clip-mask", "--accent", "--swatch", "--layer-depth", "--clip-width", "--ant-period", "--vector-zoom", "--brush-hardness", "--brush-angle", "--brush-roundness"]) defined.add(runtime);
