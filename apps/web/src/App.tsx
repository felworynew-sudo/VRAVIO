...
export function App() {
  ...
  // "Cast Shadow…"'s own point-placement status — a third parallel channel alongside
  // `transformMetrics`/`scene3dTransformMetrics` above, for the same reason those two stayed
  // separate rather than merged: never active at the same time as either, but a different enough
  // shape (a point count and a readiness flag, not X/Y/Z/W/H numbers) to keep its own.
  const [scene3dGroundMetrics, setScene3DGroundMetrics] = useState<{ active: boolean; pointCount: number; ready: boolean } | null>(null);
  ...
  const [neuralFilterSelection, setNeuralFilterSelection] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    const onScene3DGroundState = (event: Event) => setScene3DGroundMetrics((event as CustomEvent).detail ?? null);
    window.addEventListener("vravio-scene3d-ground-state", onScene3DGroundState);
    return () => window.removeEventListener("vravio-scene3d-ground-state", onScene3DGroundState);
  }, []);
      ...
      <nav aria-label={store.language === "ru" ? "Главное меню" : "Main menu"}>
        ...
        {active?.kind === "raster" && <Menu label="Image (Изображение)" language={store.language} open={openMenu === "image"} onToggle={() => setOpenMenu(openMenu === "image" ? null : "image")} items={[
          ...
          { label: "Mode (Режим)", items: [
            ...
            [`${tick(colorModel === "indexed")}Indexed Color… (${tick(colorModel === "indexed")}Индексированные цвета…)`, "", () => void kernel.commands.execute("image.mode.indexed", activeCommandContext()), !activeRasterState],
            ...
            [`${tick(colorModel === "cmyk")}CMYK Color (${tick(colorModel === "cmyk")}CMYK)`, "", () => void kernel.commands.execute("image.mode.cmyk", activeCommandContext()), !activeRasterState || colorModel === "cmyk", "Pixels are limited to a four-ink separation; the plates are derived on export, not stored (Пиксели ограничены четырёхкрасочной сепарацией; плашки выводятся при экспорте, а не хранятся)"],
            [`${tick(colorModel === "lab")}Lab Color (${tick(colorModel === "lab")}Lab)`, "", () => void kernel.commands.execute("image.mode.lab", activeCommandContext()), !activeRasterState || colorModel === "lab"],
            ...
            ["Color Table… (Таблица цветов…)", "", () => void kernel.commands.execute("image.mode.colorTable", activeCommandContext()), colorModel !== "indexed", "Only an indexed-colour document has a colour table (Таблица цветов есть только у индексированного документа)"],
        ...
        {active?.kind === "raster" && <Menu label="Layer (Слой)" language={store.language} open={openMenu === "layer"} onToggle={() => setOpenMenu(openMenu === "layer" ? null : "layer")} items={[
          ...
          { label: "Smart Objects (Смарт-объекты)", items: [
            ...
            ["Update Linked Contents (Обновить связанное содержимое)", "", () => void kernel.commands.execute("layer.updateLinkedSmartObject", activeCommandContext()), !activeRasterState || kernel.platform.kind !== "desktop" || activeRasterState.layers.find((layer) => layer.id === activeRasterState.activeLayerId)?.smartSource?.mode !== "linked"],
            ["Relink to File… (Перепривязать к файлу…)", "", () => void kernel.commands.execute("layer.relinkSmartObject", activeCommandContext()), !activeRasterState || kernel.platform.kind !== "desktop" || activeRasterState.layers.find((layer) => layer.id === activeRasterState.activeLayerId)?.smartSource?.mode !== "linked"],
            ["Embed Linked (Встроить связанный)", "", () => void kernel.commands.execute("layer.embedLinkedSmartObject", activeCommandContext()), !activeRasterState || activeRasterState.layers.find((layer) => layer.id === activeRasterState.activeLayerId)?.smartSource?.mode !== "linked"],
        ...
        {active?.kind === "raster" && <Menu label="Filter (Фильтр)" language={store.language} open={openMenu === "filter"} onToggle={() => setOpenMenu(openMenu === "filter" ? null : "filter")} items={[
          ...
          ["Neural Filters… (Нейрофильтры…)", "", () => { setNeuralFilterSelection(undefined); setNeuralFiltersOpen(true); }, !activeRasterState || activeRasterState.layers.find((layer) => layer.id === activeRasterState.activeLayerId)?.kind !== "pixel"],
          ...
          { label: "Noise (Шум)", items: [
            ...
            ["AI Denoise… (ИИ подавление шума…)", "", () => { setNeuralFilterSelection("ai-denoise"); setNeuralFiltersOpen(true); }, !activeRasterState || activeRasterState.layers.find((layer) => layer.id === activeRasterState.activeLayerId)?.kind !== "pixel"],
          ] },
          { label: "Pixelate (Пикселизация)", items: [
            ["Color Halftone… (Цветной полутон…)", "", () => openFilter("color_halftone"), !active || active.kind!=="raster"],
            ["Crystallize… (Кристаллизация…)", "", () => openFilter("crystallize"), !active || active.kind!=="raster"],
            ["Fragment (Фрагмент)", "", () => applyOneShotFilter("fragment"), !active || active.kind!=="raster"],
            ["Mezzotint… (Глубокая печать…)", "", () => openFilter("mezzotint"), !active || active.kind!=="raster"],
