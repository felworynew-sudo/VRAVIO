...
import { addShape, buildShapeSpatialIndex, createShape, createVectorDocument, emptyVectorStyle, solidFill, solidStroke, type VectorDocumentState } from "@vravio/env-vector";
...
function documentFixture(): VectorDocumentState {
  ...
  const rectangle = createShape("rectangle", 40, 40, { ...emptyVectorStyle(), fills: [solidFill(srgb(0x5b, 0xe0, 0xb3))] });
  ...
  const path = createShape("path", 200, 150, { ...emptyVectorStyle(), strokes: [solidStroke(srgb(0, 0, 0))] });
