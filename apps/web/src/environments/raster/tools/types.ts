...
// moved to `../../navigation-types` so `environments/vector/tools/types.ts`
// can share the exact same contract instead of a hand-kept duplicate that
// only looks identical today — see that file's own doc comment. Re-exported
// so `hand.ts`/`zoom.ts`/`raster-navigation.ts`/`contract.test.ts` (this
// environment's own files, importing from this module already) do not each
// need their own import path changed.
