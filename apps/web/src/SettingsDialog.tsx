      ...
      <div className="settings-layout">
        ...
        <div className="settings-content">
          ...
          {page === "guides" && <>
            ...
            <ToggleRow title={text(language, "Glow inside selections", "Свечение внутри выделения")} description={text(language, "A soft glow just inside the selection edge, under the marching ants. Its width is measured on screen and does not grow with the zoom.", "Мягкое свечение внутрь от края выделения, под марширующими муравьями. Ширина задана в экранных пикселях и не растёт при увеличении.")} checked={store.preferences.selectionGlow} onChange={(selectionGlow) => store.updatePreferences({ selectionGlow })} />
            ...
            <SettingRow title={text(language, "Snap sensitivity", "Чувствительность привязки")} description={text(language, "How close a drag has to land to a guide before it snaps, in screen pixels. Lower feels less aggressive.", "Насколько близко нужно подвести перетаскиваемый объект к направляющей, чтобы сработала привязка, в экранных пикселях. Меньше значение — мягче привязка.")}><NumberInput value={store.preferences.snapSensitivity} min={1} max={40} suffix="px" onChange={(snapSensitivity) => store.updatePreferences({ snapSensitivity })} /></SettingRow>
