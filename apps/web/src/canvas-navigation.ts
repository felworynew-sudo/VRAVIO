...
import type { NavigationContext, NavigationGesture, NavigationHooks } from "./environments/navigation-types";
 ...
 * sees it.
 *
 * Originally `raster-navigation.ts`'s own `useCanvasNavigation`, generalized
 * so `VectorWorkspace.tsx` can drive `vector.hand`/`vector.zoom` through the
 * exact same logic instead of a second copy that only looks the same today —
 * the tool lookup and the two tool ids that need host-level overrides (space
 * bar, middle mouse, the temporary zoom modifier) are now parameters instead
 * of being read straight off the raster catalogue. `raster-navigation.ts` and
 * `vector-navigation.ts` are now thin wrappers supplying those parameters, so
 * neither workspace component's own import changed.
 *
 * What lives here is everything that is not the active tool: the space bar
 * and the middle mouse button as a temporary hand, space-plus-modifier as a
 ...
 * "space is a temporary hand tool" is not an imitation of the hand tool; it
 * is the hand tool.
 */
...
export function useCanvasNavigation(params: {
  ...
  /** The environment's own tool lookup — `rasterToolById` or `vectorToolById` — read
   * only for each tool's `navigation` hooks, never for anything environment-specific. */
  toolById: ReadonlyMap<string, { navigation?: NavigationHooks }>;
  /** This environment's zoom tool id (`"raster.zoom"`/`"vector.zoom"`) — what the
   * space-plus-modifier temporary zoom drives. */
  zoomToolId: string;
  /** This environment's hand tool id — what the space bar and the middle mouse
   * button temporarily become regardless of the tool actually selected. */
  handToolId: string;
  /**
   * Skips this hook's own "fit to window" effect below. Vector documents fit
   * to the *active artboard*'s bounds (`VectorWorkspace.tsx`'s own fit
   * effect, predating this shared hook and kept as-is rather than folded in
   * here), not to a fixed document width/height the way raster's plain
   * canvas does — two effects both writing `viewport` on `mode: "fit"` would
   * fight each other. Everything else this hook offers (gesture handling,
   * workspace-size tracking, the wheel handler) has no such per-environment
   * difference and is shared unconditionally.
   */
  disableFit?: boolean;
}) {
  const { documentId, workspaceRef, viewport, activeToolId, toolOptions, documentWidth, documentHeight, toolById, zoomToolId, handToolId, disableFit } = params;
  ...
  useEffect(() => {
    ...
    if (disableFit || !workspace || viewport.mode !== "fit") return;
    const fit = () => {
  ...
  }, [documentId, setViewport, documentHeight, documentWidth, viewport.mode, viewport.rotation, disableFit]);
  ...
  const beginNavigation = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const toolId = spaceZoom ? zoomToolId : temporaryHand ? handToolId : activeToolId;
    const hooks = toolId ? toolById.get(toolId)?.navigation : undefined;
    if (!hooks) return;
  ...
  const moveNavigation = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    toolById.get(current.toolId)?.navigation?.move?.(contextFor(current.options), gestureFrom(current, event));
  };
  ...
  const endNavigation = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!current || current.pointerId !== event.pointerId) return;
    event.preventDefault(); event.stopPropagation();
    toolById.get(current.toolId)?.navigation?.end?.(contextFor(current.options), gestureFrom(current, event));
