...
// and the equivalent in the CLEAN_CANVAS_EVENT handler) — never inline on `addGroup`, and
// never in the same tick a group is created. Found live: passing `constraints` at creation
// time (or even a `requestAnimationFrame`-deferred `setConstraints` right after) corrupts
// Dockview's own fresh-layout width distribution — reproduced on the vector environment's
...
function LayerMaskThumbnail({ mask, width, height, active, onActivate, onToggleEnabled, onDragStart }: { mask: RasterLayerMask; width: number; height: number; active: boolean; onActivate(): void; onToggleEnabled(): void; onDragStart(event: React.PointerEvent): void }) {
  const ref = useRef<HTMLCanvasElement>(null);
  useEffect(() => {
    const canvas = ref.current, context = canvas?.getContext("2d"); if (!canvas || !context) return;
  ...
  // Shift-click disables/enables the mask, as in Photoshop; a disabled mask is
  // crossed out (red X) the way Photoshop marks it.
  return <span className={`layer-mask-thumb${active ? " editing" : ""}${mask.enabled ? "" : " disabled"}`} title="Edit Layer Mask (Редактировать маску слоя)" onClick={(event) => { event.stopPropagation(); if (event.shiftKey) onToggleEnabled(); else onActivate(); }} onPointerDown={onDragStart}><canvas ref={ref} width="28" height="28"/></span>;
}
...
const layerEffectDefaults: RasterLayer["effects"] = {
  ...
  outerGlow: { enabled: false, color: "#ffffff", opacity: .6, radius: 6 }, innerGlow: { enabled: false, color: "#ffffff", opacity: .5, radius: 5 }, bevel: { enabled: false, strength: .65 }, gradientOverlay: { enabled: false, from: "#8f5cff", to: "#56d8ff", opacity: .7, angle: 0 }, glass: { enabled: false, blur: 18, tintOpacity: .18, invertLuminance: false },
};
...
const LAYER_STYLE_SECTIONS = [["blending", "Blending Options (Параметры смешивания)"], ["glass", "Glass (Стекло)"], ["bevel", "Bevel & Emboss (Рельеф и тиснение)"], ["innerShadow", "Inner Shadow (Внутренняя тень)"], ["innerGlow", "Inner Glow (Внутреннее свечение)"], ["gradientOverlay", "Gradient Overlay (Наложение градиента)"], ["outerGlow", "Outer Glow (Внешнее свечение)"], ["dropShadow", "Drop Shadow (Тень)"]] as const;
...
function LayerStyleDialog({ layer, onApply, onClose }: { layer: RasterLayer; onApply(patch: Partial<RasterLayer>): void; onClose(): void }) {
  ...
  return <ModalBackdrop className="layer-style-backdrop" onMouseDown={onClose}><section className="layer-style-dialog" role="dialog" aria-modal="true" aria-label="Layer Style (Стиль слоя)" onMouseDown={(event) => event.stopPropagation()}>
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      void (async () => {
        ...
        if (!confirmed) return;
        // The removal itself is the catalogue's `layer.deleteMask` (it targets
        // the mask being edited, which is this one), shared with the menu and
        // the Contextual Task Bar; only the confirmation belongs to the key.
        await kernel.commands.execute("layer.deleteMask", { activeDocumentId: documentId });
      })();
    };
  ...
  if (active && isRasterDocumentState(active.state)) {
    ...
    const layerContextMenu = (layer: RasterLayer): ContextMenuItem[] => {
      ...
      return [
        ...
        { label: text(language, "Apply Mask", "Применить маску"), onSelect: () => runMaskCommand(layer, "layer.applyMask"), disabled: layer.kind === "group" || !layer.mask, separatorBefore: true },
        { label: text(language, layer.mask?.enabled === false ? "Enable Layer Mask" : "Disable Layer Mask", layer.mask?.enabled === false ? "Включить маску слоя" : "Выключить маску слоя"), onSelect: () => runMaskCommand(layer, "layer.toggleMaskEnabled"), disabled: layer.kind === "group" || !layer.mask },
        { label: text(language, "Delete Layer Mask", "Удалить маску слоя"), onSelect: () => runMaskCommand(layer, "layer.deleteMask"), disabled: layer.kind === "group" || !layer.mask },
    ...
    // Add Layer Mask lives in the command catalogue (`layer-mask.ts`) — the
    // Contextual Task Bar's "Create mask" reaches the same operation, and a
    // local copy here would be the second door CLAUDE.md §4 warns about.
    const addMask = () => void kernel.commands.execute("layer.addMask", { activeDocumentId: active.id });
    ...
    // Apply / Disable / Delete Layer Mask live in the command catalogue
    // (`layer-mask.ts`), shared with the Layer menu and the Contextual Task
    // Bar. The row's menu first makes the clicked layer the target — the
    // commands act on the mask being edited, else the active layer.
    const runMaskCommand = (layer: RasterLayer, id: string) => {
      selectLayer(layer.id);
      if (editingMaskLayerId && editingMaskLayerId !== layer.id) setEditingMask(active.id, null);
      void kernel.commands.execute(id, { activeDocumentId: active.id });
    };
    ...
    return <div className="dock-panel-body layers-panel">
      ...
      <div className="layer-controls"><select value={activeLayer.effects?.glass?.enabled ? "__glass__" : activeLayer.blendMode} onChange={(event) => {
        if (event.target.value === "__glass__") { updateActive({ effects: { ...activeLayer.effects, glass: { ...layerEffectDefaults.glass!, enabled: true } } }); return; }
        updateActive({ blendMode: event.target.value as RasterBlendMode, ...(activeLayer.effects?.glass?.enabled ? { effects: { ...activeLayer.effects, glass: { ...activeLayer.effects.glass, enabled: false } } } : {}) });
      }}>{blendModes.map((mode) => <option key={mode} value={mode}>{mode}</option>)}<optgroup label="──────── Effects / Эффекты ────────"><option value="__glass__">Glass (Стекло)</option></optgroup></select><div className="layer-controls-row"><label><span>{text(language, "Opacity", "Непрозр.")}</span><input type="number" min="0" max="100" value={Math.round(activeLayer.opacity * 100)} onChange={(event) => updateActive({ opacity: Math.max(0, Math.min(1, event.target.valueAsNumber / 100)) })}/><i>%</i></label><label><span>{text(language, "Fill", "Заливка")}</span><input type="number" min="0" max="100" value={Math.round((activeLayer.fillOpacity ?? 1) * 100)} onChange={(event) => updateActive({ fillOpacity: Math.max(0, Math.min(1, event.target.valueAsNumber / 100)) })}/><i>%</i></label></div></div>
...
function PanelHeaderActions({ api, containerApi, activePanel, group, panels }: IDockviewHeaderActionsProps) {
  ...
  const moveActiveToNewGroup = (direction: "above" | "below") => {
    ...
    // No `constraints` here: passing one inline to `addGroup` (or even via a same-frame
    // follow-up `setConstraints`) on a group that's still in its very first layout pass
    // corrupts Dockview's own width distribution — found live on the vector environment's
    // default 4-panel layout, which grew to ~850px while the canvas shrank to ~380px, and
    // reproduced identically here. The two toggle-driven `setConstraints` calls below (see
    // `toggleCollapsed`/`CLEAN_CANVAS_EVENT`) are safe because they run long after their
    // group's first layout has already settled, on a real user click — never in the same tick
    // a group is created.
...
function createDefaultLayout(api: DockviewReadyEvent["api"], language: Language, kind: EnvironmentKind, panelIds: readonly string[]): void {
  ...
  // `initialWidth` alone was not landing on a fresh two-group grid split — found live on both
  // raster and vector's own default layout, each settling at an even 50/50 split of the whole
  // workspace width instead of the requested 280px side column (reproducible on a cold
  // server, no saved layout, no prior session state). An explicit `setSize` after the split
