...
function LayersPanel() {
  ...
  if (active && isRasterDocumentState(active.state)) {
    ...
    return <div className="dock-panel-body layers-panel">
      ...
      <div className="layer-list">{(() => { const visibleRows = rasterLayerRows(state.layers).filter(({ layer }) => !layerFilterOn || layerFilter === "all" || layer.kind === layerFilter || layer.kind === "group"); return visibleRows.map(({ layer, depth }, index) => {
