...
export function import_svg(svg_text: string): string;
 ...
 */
export function render_svg_to_rgba(svg_text: string, width: number, height: number): Uint8Array;

export type InitInput = RequestInfo | URL | Response | BufferSource | WebAssembly.Module;

export interface InitOutput {
    readonly memory: WebAssembly.Memory;
    readonly import_svg: (a: number, b: number) => [number, number, number, number];
    readonly render_svg_to_rgba: (a: number, b: number, c: number, d: number) => [number, number, number, number];
