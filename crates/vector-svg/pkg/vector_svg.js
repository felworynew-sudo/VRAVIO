 ...
 * @returns {Uint8Array}
 */
export function render_svg_to_rgba(svg_text, width, height) {
    const ptr0 = passStringToWasm0(svg_text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.render_svg_to_rgba(ptr0, len0, width, height);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('vector_svg_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
