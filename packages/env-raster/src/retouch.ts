...
function insideBrush(x: number, y: number, point: Point, radius: number, roundness: number, angle: number): number {
...
export function blurDab(pixels: Uint8ClampedArray, source: Uint8ClampedArray, width: number, height: number, point: Point, size: number, strength: number, selectionMask?: Uint8ClampedArray, roundness = 1, angle = 0): void {
  ...
  for (let y = top; y <= bottom; y += 1) for (let x = left; x <= right; x += 1) {
    const coverage = insideBrush(x, y, point, radius, roundness, angle); if (!coverage) continue;
...
export function blurStrokeSegment(pixels: Uint8ClampedArray, source: Uint8ClampedArray, width: number, height: number, from: Point, to: Point, size: number, strength: number, selectionMask?: Uint8ClampedArray, roundness = 1, angle = 0): void {
  ...
  for (let y = effectTop; y <= effectBottom; y += 1) for (let x = effectLeft; x <= effectRight; x += 1) {
    ...
    const center = { x: from.x + dx * projection, y: from.y + dy * projection }, coverage = insideBrush(x, y, center, radius, roundness, angle); if (!coverage) continue;
    const selection = selectionMask ? selectionMask[y * width + x]! / 255 : 1; if (!selection) continue;
...
export function dodgeBurnDab(pixels: Uint8ClampedArray, width: number, height: number, point: Point, size: number, strength: number, mode: "dodge" | "burn", range: DodgeBurnRange, selectionMask?: Uint8ClampedArray, roundness = 1, angle = 0): void {
...
export function dodgeBurnStrokeSegment(pixels: Uint8ClampedArray, width: number, height: number, from: Point, to: Point, size: number, strength: number, mode: "dodge" | "burn", range: DodgeBurnRange, selectionMask?: Uint8ClampedArray, roundness = 1, angle = 0): void {
  const dx = to.x - from.x, dy = to.y - from.y, distance = Math.hypot(dx, dy), steps = Math.max(1, Math.ceil(distance / Math.max(1, size * .2)));
  for (let step = 1; step <= steps; step += 1) { const t = step / steps; dodgeBurnDab(pixels, width, height, { x: from.x + dx * t, y: from.y + dy * t }, size, strength, mode, range, selectionMask, roundness, angle); }
}

export function smudgeStrokeSegment(pixels: Uint8ClampedArray, source: Uint8ClampedArray, width: number, height: number, from: Point, to: Point, size: number, strength: number, selectionMask?: Uint8ClampedArray, roundness = 1, angle = 0): void {
  const dx = to.x - from.x, dy = to.y - from.y, distance = Math.hypot(dx, dy), steps = Math.max(1, Math.ceil(distance / Math.max(1, size * .14))), radius = Math.max(.5, size / 2);
  for (let step = 1; step <= steps; step += 1) { const t = step / steps, point = { x: from.x + dx * t, y: from.y + dy * t }, sourcePoint = { x: point.x - dx / steps, y: point.y - dy / steps }, left = Math.max(0, Math.floor(point.x - radius)), right = Math.min(width - 1, Math.ceil(point.x + radius)), top = Math.max(0, Math.floor(point.y - radius)), bottom = Math.min(height - 1, Math.ceil(point.y + radius));
    ...
    for (let y = top; y <= bottom; y += 1) for (let x = left; x <= right; x += 1) { const coverage = insideBrush(x, y, point, radius, roundness, angle); if (!coverage) continue; const selection = selectionMask ? selectionMask[y * width + x]! / 255 : 1; if (!selection) continue; const sx = Math.max(0, Math.min(width - 1, Math.round(sourcePoint.x + x - point.x))), sy = Math.max(0, Math.min(height - 1, Math.round(sourcePoint.y + y - point.y))); mixPixel(pixels, (y * width + x) * 4, smearSource, (sy * width + sx) * 4, strength * coverage * selection); }
  }
}
