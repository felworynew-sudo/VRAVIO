...
- [ ] **4.** `Ctrl+Shift+I` — горячая клавиша «инвертировать выделение»
      (`select.invert` уже существует в командах, см. `catalogue.test.ts`)
      — проверить, что назначена именно эта комбинация и работает
      предсказуемо рядом с новым поведением п. 3 (не конфликтует).
- [ ] **5.** Маску можно схватить мышью (drag) прямо с миниатюры в
      панели слоёв и перетащить на другой слой — перемещает маску на
      слой назначения (Photoshop: обычный drag миниатюры маски между
      строками панели слоёв).
