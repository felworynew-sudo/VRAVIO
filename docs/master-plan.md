...
- [ ] **12.** Подтверждающий диалог удаления маски (пункт 1) — добавить
      чекбокс «Больше не спрашивать» прямо в диалог. Это должно быть не
      разовый локальный флаг, а кусочек общих настроек приложения (окно
      «Настройки»): раздел с перечнем «спрашивать при …» — «удалении
      маски», и по мере появления новых подобных подтверждающих
      диалогов — туда же, одним списком, а не отдельным флагом на
      каждый диалог сам по себе. Нужно, чтобы это можно было включить
      обратно, если пользователь передумает — то есть настройка
      двусторонняя (выключить/включить), не одноразовое «навсегда
      скрыть». Архитектурно: один общий кусок состояния (например,
      `store.ts`'s `confirmPreferences: Record<string, boolean>` или
      похожее), который `confirmModal` читает по ключу диалога, а не
      отдельная настройка на каждый вызывающий код — та же
      «единственная дверь», что и для остальных подобных списков в этом
      документе.
