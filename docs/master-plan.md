...
- [ ] **12.** Подтверждающий диалог удаления маски (пункт 1) — добавить
      ...
      документе.
- [ ] **13.** Если в группе инструментов на панели развернуть флайаут и
      выбрать конкретный инструмент, а потом ещё раз выбрать другой
      инструмент внутри той же группы — главная (видимая без разворота)
      иконка группы всё равно продолжает показывать первый инструмент
      группы, не последний выбранный. `ToolPalette` в `App.tsx`
      (`const selected = group.find((tool) => tool.id === activeToolId)
      ?? group[0]!`) вроде бы уже должен показывать активный —
      проверить, почему на практике этого не происходит (возможно,
      `activeToolId` не обновляется для скрытых во флайауте
      инструментов, или что-то сбрасывает выбор при закрытии флайаута).
      Отдельно: по умолчанию первым инструментом в группе `J` должна
      быть точечная восстанавливающая кисть (Spot Healing Brush), не
      что сейчас стоит первым — сверить с `tools.ts`'s текущим порядком
      группы и с default toolbar layout (`toolbar/layout.ts`).
