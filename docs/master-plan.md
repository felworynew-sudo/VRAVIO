...
**Статус: сознательно отложено владельцем на самый конец очереди** (после
Hand/Zoom, Curvature Tool, Shape Builder) — сессия 6 сентября 2026,
сообщение «рефакторинг ручек по архитектуре svgedit... можешь записать
подробно в мастер плане». Это запись того, что нужно сделать, а не
выполненная работа.
...
**Объём работы, если браться всерьёз (не только `.vector-handle`):**

- [ ] Новый немасштабируемый `<svg>` (или `<div>` с абсолютным
      позиционированием), брат `.vector-stage`, а не его потомок —
      аналог `.selection-overlay` из растровой стороны
      (`apps/web/src/styles.css`, `position:absolute;inset:0`).
- [ ] Функция «документные координаты → экранные пиксели», учитывающая
      pan/zoom/rotation — уже частично существует как обратное
      преобразование в `VectorWorkspace.tsx`'s `toDocumentPoint`/
      `toDocumentPointRaw` (сейчас только в одну сторону, экран→документ,
      для хит-тестов); нужна прямая версия документ→экран.
- [ ] `.vector-selection` + `.vector-handle` (сама рамка выделения и 4
      угловые ручки, `VectorWorkspace.tsx`) — перенести первыми, это
      единственное, на что владелец уже пожаловался живым багом.
- [ ] `nodes.tsx`'s `Overlay` (анкоры, ручки Безье, линии ручка↔анкор) —
      сложнее: сейчас хит-тестирование клика по анкору/ручке
      (`hitTestNode`, `applyNodeMove`) работает в документных
      координатах, и перенос рендера в экранный слой не должен менять
      эту логику — только рендер, не геометрию/хит-тест.
- [ ] `pen.tsx`/`curvature.tsx`'s резиновая линия предпросмотра
      (`.vector-pen-rubber-band`) — сама линия тянется до курсора и
      *должна* визуально идти от фигуры до курсора, то есть часть её
      геометрии в документных координатах, часть — в экранных; нужно
      решить, красиво ли это разложить на два слоя, или просто оставить
      как есть (она уже использует `1/zoom` и не имеет обнаруженных
      багов).
- [ ] `.vector-snap-guide` (линии привязки, `select.tsx`) — тот же вопрос,
      что и выше.
- [ ] Контрактные и regression-тесты, которые уже опираются на текущую
      геометрию Overlay (если такие есть — не проверено в этом заходе).

**Не начинать это до тех пор, пока Hand/Zoom (сделано), Curvature Tool
(сделано) и Shape Builder не будут закончены** — прямое указание
владельца в этой же сессии.
