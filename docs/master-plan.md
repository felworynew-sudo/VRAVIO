...
- [ ] **9.** Обтравочная маска (clipping mask) — горячая клавиша
      ...
      - `Alt`/`Option` + клик на границе между базовым слоем снизу и
        слоем над ним в панели слоёв (курсор — пересекающиеся круги);
      - пункт меню `Слой → Создать обтравочную маску`.
      Проверить, что команда `layer.*` для этого уже существует в
      catalogue (`grep` по `clip` в `apps/web/src/commands/`) — если да,
      это подключение хоткея/меню/жеста к существующей логике, а не
      новая реализация обтравки с нуля.
