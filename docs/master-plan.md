...
### 29.5. Не проверено в этой сессии: drag-and-drop файла на Mac

Владелец также сообщил, что drag-and-drop файла в браузере на его MacBook не работает, и отдельно предупредил, что причина может быть в настройках самого Mac, а не в VRAVIO — просил перепроверить. Эта среда — Windows, реального Mac/Safari нет; код обработчика drag-and-drop не читался и не менялся в этой сессии за нехваткой способа проверить исправление живым вводом на затронутой платформе (CLAUDE.md §2 — не сообщать о результате, которого не видел). Остаётся открытой задачей: нужен либо доступ к Mac для живой проверки, либо отчёт владельца с более точными деталями (какой браузер на Mac, точные шаги, есть ли ошибка в консоли) прежде чем чинить вслепую.
