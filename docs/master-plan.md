...
**Не сделано из этого захода**: экспорт (тот же decode/encode-конвейер,
что и `extractAsset`), реалтайм-плагины для видео (`environments/
...
Web Audio микшер для встроенного звука клипов, бесшовный переход между
клипами/дорожками при воспроизведении (сейчас каждый активный клип на
каждой дорожке действительно проигрывается и композитится в реальном
