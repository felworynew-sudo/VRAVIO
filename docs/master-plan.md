...
- [ ] **10.** Кнопка «Связать слои» не работает. Связанные слои должны
      ...
      уже есть `toggleLayerLink`/`layer.linkGroup` (используется в
      `layerContextMenu` в `DockLayout.tsx`) — проверить, реализовано ли
      совместное перемещение в самом инструменте перемещения
      (`environments/raster/tools/definitions/move.tsx`) или только
      флаг на слое без эффекта (CLAUDE.md раздел 3: настройка обязана
      влиять на результат).
