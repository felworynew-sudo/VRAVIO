...
- [x] Video Track
- [x] Audio Track
- [x] Clip { source, timelineStart, sourceIn, sourceOut, duration } — без
      `position, scale, opacity, crop` (композитинг/трансформация клипа,
      см. ниже)
- [x] timeline position
- [x] trim
- [x] split (blade)
...
**Не сделано из этого захода**: snapping (привязка к границам соседних
клипов при перетаскивании), transform (position/scale/opacity клипа —
нужен реальный компоузинг-конвейер), crop, экспорт (тот же decode/encode-
конвейер, что и `extractAsset`), реалтайм-плагины для видео (`environments/
...
поверхности плагинов — легитимный пустой ответ, а не ошибка), реальное
многодорожечное воспроизведение с бесшовным переходом между клипами.
