...
- [x] **Из Audacity 4 — realtime effect stack, ripple editing,
      ...
      - **Ripple editing.** `rippleShift` в `clip-operations.ts` —
        ...
        удаление закрывало свой пробел до следующего. **Не сделано**:
        ripple при обрезке края клипа (trim) — усложнило бы живой
        drag-preview (пришлось бы двигать чужие клипы прямо во время
        перетаскивания, не только на коммите), отложено как отдельная
        более сложная задача; ripple пока — только про удаление.
...
- [x] **Automation — громкость дорожки во времени (8 сентября 2026, тот
      ...
      **Не сделано из всего Audacity-4 этапа**: plugin architecture,
      advanced recording (punch-in, множественные дубли), ripple при
      обрезке края клипа (см. заметку в разделе ripple выше),
      автоматизация произвольных параметров эффектов (не только
      громкости дорожки) — следующий заход.
