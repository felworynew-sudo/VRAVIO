...
- [ ] **11.** Индикатор применённого стиля слоя (Layer Style) в строке
      слоя — сейчас, судя по всему, отсутствует. Нужно:
      - иконка «эффект» (fx) появляется на строке слоя, если у слоя есть
        хоть один применённый стиль (`RasterLayerEffects`);
      - клик по иконке — открывает диалог стиля слоя (уже есть,
        `setStyleLayerId`), при этом иконка тускнеет, показывая, что
        диалог открыт;
      - правее иконки — отдельная стрелка раскрывающегося списка,
        разворачивающая ПОД строкой слоя список применённых эффектов:
        каждая строка — иконка глаза + название эффекта (Drop Shadow,
        Inner Glow и т. д., только реально применённые, не весь
        каталог); клик по иконке глаза в этом списке скрывает/показывает
        именно этот эффект по отдельности, не весь стиль целиком —
        сверить, есть ли уже в `RasterLayerEffects` поле
        `enabled`/`visible` на КАЖДОМ эффекте отдельно, а не только на
        стиле в целом.
