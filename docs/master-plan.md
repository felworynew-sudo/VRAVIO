...
- [ ] **2.** Если на холсте есть активное выделение и в этот момент
      создаётся маска слоя — сейчас выделение, видимо, просто пропадает.
      Должно быть: выделение конвертируется в форму маски (маска красится
      в белый там, где было выделение, в чёрный — где не было), а не
      теряется.
- [ ] **3.** `Ctrl+I` (инверсия цветов, уже существующая горячая клавиша)
      должен работать и на маске, когда активна миниатюра маски — то есть
      инвертировать саму маску (чёрное/белое), а не пытаться
      инвертировать цвета пиксельного слоя.
