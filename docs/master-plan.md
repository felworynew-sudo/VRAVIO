...
- [ ] **6.** Правый клик по слою → контекстное меню — добавить пункты:
      «Применить маску», «Скопировать маску», «Вставить маску»,
      «Скопировать стиль слоя», «Вставить стиль слоя», «Применить стиль
      слоя». Сверить точные русские подписи с тем, как уже называются
      соседние пункты этого же меню (CLAUDE.md раздел 4 уже поймал один
      разъехавшийся дубль подписи для похожего меню — не плодить второй).
...
## 19. Маски слоя — полный разбор архитектуры против Patchy, 7 сентября 2026

Владелец принёс развёрнутый разбор (реальное чтение обоих кодовых баз —
`packages/env-raster/src` VRAVIO и `src/core`/`src/ui` Patchy). Записано
здесь целиком по существу, со сжатием риторики, но без потери фактов —
это будущий рабочий план по маскам, не разовая справка.

### 19.1. Итоговая оценка

**Patchy — 9/10**: цельная Photoshop-подобная система (layer/group/
vector/filter masks, Quick Mask, channels, overlay, view, invert,
disable, link, apply, PSD semantics). **VRAVIO — 5/10 по готовности,
но ~7/10 по заложенному фундаменту**: модель данных `RasterLayerMask`
местами богаче, чем у Patchy (`density` — честное live-свойство
компоузера, чего у Patchy нет вовсе; общий `PaintTarget`, не второй
набор инструментов для маски), но добрая половина полей —
задел на будущее, ещё не подключённый к интерфейсу.

### 19.2. КРИТИЧЕСКИЙ найденный баг — инверсия маски и рисование расходятся

**Подтверждено прямым чтением кода в этой же сессии** (не только
разбором владельца): `apps/web/src/raster-pixel-buffers.ts`'s
`maskToRgba`/`rgbaToMask` — путь, которым `RasterWorkspace.tsx` кормит
любой инструмент рисования, когда активна маска (`paintTarget.kind ===
"mask"`) — читают и пишут **сырые** `mask.pixels`, никак не учитывая
`mask.inverted`. А компоузер (`render.ts`, `layer-tree.ts`) применяет
`inverted` только на экране: `inverted ? 255 - pixel : pixel`.

Результат: если `mask.inverted === true`, кисть белым цветом пишет
`255` в `mask.pixels`, что компоузер тут же превращает в `255-255=0` —
то есть **белая кисть скрывает, а не показывает**, ровно наоборот
ожидания «белый — видно, чёрный — скрыто». До этой сессии баг был
латентным — пользовательской команды переключения `mask.inverted` не
было нигде. **Она появилась только что** (раздел 1.9, пункт 3 — этот
самый мастер-план, `Ctrl+I` на маске), то есть баг стал достижимым
прямо сейчас, этим же изменением.

- [x] **Решение принято и реализовано сразу же после записи этого
      раздела** (см. git-историю следом за этим коммитом): убрать
      `inverted` как постоянное состояние, сделать «Инвертировать
      маску» **физической** операцией — `pixels[i] = 255 - pixels[i]`
      по всему буферу, как у Patchy (`for (auto& value : mask.pixels)
      value = 255 - value;`, с undo-снапшотом до). После этого
      `mask.inverted` в типе `RasterLayerMask` — мёртвое поле,
      компоузер про него забывает, и вся система (кисть, заливка,
      градиент, thumbnail, mask→selection, экспорт) не должна больше
      нигде помнить о логической инверсии. Цена — O(width×height) на
      клик «Инвертировать», не 60 раз в секунду, приемлемо даже на
      больших документах — та же логика, что уже применяется к
      `willChange`/other perf-decisions в этом документе: мерить, а
      не бояться заранее.

### 19.3. Сводная таблица — что есть, чего нет
...
| Disable/Enable маски в UI | ⚠️ `mask.enabled` в компоузере есть, переключателя в UI нет | ✅ |
| Invert | ⚠️ было флагом с багом рисования (см. 19.2), чинится на физическую операцию | ✅ физическая |
| Density (недеструктивный) | ✅ **VRAVIO впереди** — live-свойство компоузера | ❌ нет как first-class свойства |
| Feather | ⚠️ поле есть, `render.ts` его **не читает вообще** — сейчас чистый placeholder | ❌ нет как отдельного свойства |
| Link/Unlink маски и слоя | ⚠️ поле + иконка цепи в thumbnail есть, полного поведения (двигать маску вместе со слоем) не подтверждено | ✅ полностью, с собственными `x/y` у маски |
| Независимые bounds/defaultValue маски (не canvas-size) | ❌ маска всегда размером с документ | ✅ `LayerMask{bounds, default_color}` — экономия памяти на маленьких масках на больших холстах |
| Overlay маски (красным поверх изображения) | ❌ нет | ✅ |
| Solo grayscale view (Alt-клик по thumbnail) | ❌ нет | ✅ |
| Apply Mask (запечь в альфу, удалить маску) | ❌ нет | ✅ |
| Group masks (включая Pass Through) | ❌ явно запрещено (`layer.kind === "group"` блокирует `addMask`) | ✅, включая корректную семантику для Pass Through групп |
| Quick Mask | ❌ нет | ✅ (но `PixelSelection.mask` — уже готовый фундамент, см. 19.6) |
| Saved Alpha/Spot Channels на уровне документа | ❌ нет | ✅ |
| Vector Mask (отдельная сущность от Raster Mask) | ❌ нет | ✅ |
| Smart Filter mask | ❌ нет (недеструктивных фильтров пока нет вообще, раздел 6.4) | ✅ |

### 19.4. Почему `density` у VRAVIO — правильный образец, а `feather` — нет

`density` уже воплощает верный паттерн: `maskAlpha = rawMaskAlpha ×
density` в компоузере, сама маска не трогается. Это ровно то, что
нужно сделать для `feather`, но пока не сделано:

```
raw mask → (инверсия — теперь физическая, не флаг) → cached blur(feather) → × density → компоузер
```

**Не блюрить весь буфер на каждое движение слайдера или мазок кисти.**
Кэш ключуется на `(mask revision, feather, dimensions)`, сбрасывается
только когда меняются сами пиксели/feather/размер документа; для
кисти — dirty-region blur с паддингом `ceil(3σ)` вокруг изменённой
области, не весь холст.

### 19.5. Инвалидация рендер-кэша — отдельный, уже существующий технический долг

`layerRenderSignatures()` (упомянуто в разборе, файл кэша инвалидации
компоузера) учитывает `mask.pixels`/`maskEnabled`, но **не** учитывает
`mask.density`/`mask.feather` (и не должен больше учитывать
`inverted`, если тот становится физической операцией по 19.2). Значит
изменение density или будущего feather может не долетать до некоторых
inkremental-repaint путей. Чинить вместе с самим Feather (19.4), не
раньше — сейчас `density` уже как-то работает по факту (вероятно,
через более грубую инвалидацию где-то выше), но подтверждать/чинить
сигнатуру нужно тем же заходом.

### 19.6. Group Masks — самый нужный структурный пробел

Наивная реализация («отрендерить детей группы во временное изображение,
применить маску группы, скомпоновать») ломается для **Pass Through**
групп: adjustment-слой внутри такой группы обязан видеть задний план
ЗА пределами группы, а не только содержимое группы. Donor-подход
Patchy: применять маску группы к вкладу **каждого ребёнка** отдельно
при обходе дерева, вложенные group masks перемножаются —
`parentMask × childMask × layerAlpha`. Делать по этому образцу, не
изобретать плоский flatten.

### 19.7. Quick Mask — самая дешёвая победа из всего раздела

У нас уже есть `PixelSelection.mask` (тот же grayscale-формат, что и
`RasterLayerMask.pixels`, уже подтверждено переиспользованием в §1.9
пункте 2 этого документа). Quick Mask концептуально — не новая
подсистема, а **режим редактирования того же `selection.mask`** как
временного `PaintTarget`, с красным overlay поверх изображения, вход/
выход по `Q`, как в Photoshop. Общий `PaintTarget`-пайплайн (уже
существующий) справится без изменений — нужен только режим и оверлей.

### 19.8. Что НЕ делать сейчас

Явно вне текущего скоупа, даже когда обычные маски будут готовы:
...
маски. Разбор called это прямо: «если Invert/Apply/Group Mask/Link/
Feather ещё не работают идеально, добавлять туда SAM — это
строительство второго этажа без лестницы».

### 19.9. План по приоритетам (дословно, P0→P4)

**P0 — фундамент (делать в первую очередь, до любого нового UI):**
- [x] Invert — физическая операция, не флаг (19.2 — решение принято и
      начато сразу).
- [ ] Apply Mask (запечь альфу, удалить маску, RGB-слой становится
      RGBA если не был).
- [ ] Disable/Enable маски — переключатель в UI поверх уже готового
      `mask.enabled` в компоузере.
- [ ] Delete Mask — уже сделано (§1.9 пункт 1 этого документа).
- [ ] Density — уже готово (компоузер), проверить сигнатуру рендер-кэша
      (19.5).
- [ ] Настоящий живой Feather (19.4), не placeholder-поле.
- [ ] Починить инвалидацию рендер-кэша под density/feather (19.5).
- [ ] Alt-клик по thumbnail маски → solo grayscale view; Shift-клик →
      Disable; overlay поверх изображения.

**P1 — законченный базовый UX:**
- [ ] Правый клик по mask thumbnail: Disable/Delete/Apply/Invert/Link-
      Unlink маски — отдельное контекстное меню специально для
      миниатюры маски (шире, чем уже запрошенное общее меню слоя в
      §1.9 пункте 6 — то относится к слою целиком, это — конкретно к
      маске).
- [ ] `mask bounds + defaultValue` вместо canvas-size буфера (экономия
      памяти на маленьких масках на больших документах — тот же
      принцип, что уже дал 166→3.1 МБ для границ самого слоя,
      CLAUDE.md раздел 1).
- [ ] Настоящий Link/Unlink (маска физически двигается вместе со слоем
      при `linked === true`, остаётся в документных координатах при
      `false`) — сейчас только иконка и поле, без подтверждённого
      поведения.
- [ ] Mask overlay (красным).
- [ ] Group Masks по донорской модели Pass Through (19.6).
- [ ] Quick Mask (19.7).
...
- [ ] Vector Mask как отдельный тип, не запихнутый в
      `RasterLayerMask`: `vectorMask?: RasterVectorMask` рядом с
      `mask?: RasterLayerMask` на слое. Особая возможность VRAVIO,
      которой нет у Patchy: раз у нас Vector — полноценная соседняя
      среда, а не мини-движок внутри Raster, `Edit Mask in Vector`
      может буквально открывать редактирование в векторном workspace
      через тот же asset-revision механизм, а не дублировать
      геометрию.
- [ ] PSD-совместимость масок при импорте/экспорте (отдельный большой
      проект, см. предупреждение ниже — не ставить целью №1).

**P3 — обгон Patchy, не только паритет:**
- [ ] **Mask Stack**: у слоя не одна маска, а комбинация узлов —
      ...
      покрытие взялось.
- [ ] **Reusable Mask Assets**: маска как объект `AssetStore`
      (`Mask Asset 148`), используемый несколькими слоями/средами
      одновременно — правка в одном месте обновляет все использования,
      `Duplicate Mask` и `Link Mask` — разные, явные действия.
- [ ] **Procedural Masks**: Luminosity (с диапазоном/softness/invert,
...
**P4 — уникальность VRAVIO (то, чего в принципе не может быть у
однo-средового Patchy):**
...
### 19.10. Отдельное предупреждение — не путать «догнать» и «повторить PSD»

Patchy заточен именно под точный PSD/PSB round-trip (masks, groups,
clipping, channels, vector content, Smart Objects/Filters). Наш же ТЗ
(раздел 7 этого документа) сам фиксирует PSD как «пока чтение базовых
слоёв», без записи adjustment/text/vector-данных. **Догнать Patchy по
PSD-совместимости — отдельный, огромный проект, не цель №1.**
Реалистичная стратегия: очень хороший PSD-импорт + достаточно хороший
PSD-экспорт + собственный формат без ограничений PSD — не пять лет
побайтного повторения Photoshop, которым Patchy уже специализированно
занимается.

### 19.11. Что у VRAVIO уже лучше Patchy — не терять при рефакторинге

Три решения, которые фактурно **сильнее** донора, беречь при любых
последующих правках этого раздела:
...
2. **`density` как live-свойство компоузера**, не запечённое в
   пиксели — эталонный паттерн для будущего `feather` (19.4).
